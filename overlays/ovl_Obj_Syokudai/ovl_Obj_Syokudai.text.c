...
void ObjSyokudai_Init(Actor *thisx, GlobalContext *globalCtx) {
    ...
    Collider_InitAndSetCylinder(globalCtx, &this->standCollider, &this->actor, &sStandColliderInit);
    this->standCollider.base.colType = *(&sColTypes + ((s32) this->actor.params >> 0xC));
    Collider_InitAndSetCylinder(globalCtx, &this->flameCollider, &this->actor, &sFlameColliderInit);
...
void ObjSyokudai_Update(Actor *thisx, GlobalContext *globalCtx) {
    ...
    if (this->pendingAction != 0) {
        ...
        } else if (func_800F22C4((s16) temp_a0, &this->actor) != 0) {
            this->snuffTimer = -1;
        ...
        if ((func_800CA1E8(globalCtx, &globalCtx->colCtx, this->actor.world.pos.x, this->actor.world.pos.z, &sp60, &sp64) != 0) && ((sp60 - this->actor.world.pos.y) > 52.0f)) {
            ...
            } else if (globalCtx->actorCtx.actorList[2].first->unk147 == 7) {
                sp54 = globalCtx->actorCtx.actorList[2].first;
                sp4C = 0;
                sp48 = 0;
                sp74 = temp_t2 & 0xF;
                Math_Vec3f_Diff(globalCtx->actorCtx.actorList[2].first + 0xB9C, &this->actor.world.pos, &sp3C);
                temp_f0 = sp3C.y - 67.0f;
                temp_f6 = temp_f0 * temp_f0;
                sp3C.y = temp_f0;
                phi_t0 = 0;
                phi_t1 = 0U;
                if (((sp3C.x * sp3C.x) + temp_f6 + (sp3C.z * sp3C.z)) < 400.0f) {
                    phi_t0 = -1;
                }
            }
            phi_t2 = temp_t2 & 0xF;
            if (phi_t0 != 0) {
                if (this->snuffTimer != 0) {
                    if (phi_t0 < 0) {
                        temp_v0_2 = globalCtx->actorCtx.actorList[2].first->unkB28;
                        if (temp_v0_2 == 0) {
                            globalCtx->actorCtx.actorList[2].first->unkB28 = 0xD2;
                            sp74 = temp_t2 & 0xF;
                            func_8019F1C0(&this->actor.projectedPos, 0x2822U);
                            goto block_42;
                        }
                        if ((s32) temp_v0_2 < 0xC8) {
                            globalCtx->actorCtx.actorList[2].first->unkB28 = 0xC8;
                            goto block_42;
                        }
...
block_42:
                ...
                } else if ((sp6C != 0) && (((phi_t0 > 0) && ((phi_t1 & 0x800) != 0)) || ((phi_t0 < 0) && (globalCtx->actorCtx.actorList[2].first->unkB28 != 0)))) {
                    if ((phi_t0 < 0) && ((s32) globalCtx->actorCtx.actorList[2].first->unkB28 < 0xC8)) {
                        globalCtx->actorCtx.actorList[2].first->unkB28 = 0xC8;
                    }
                    if ((temp_t2 & 0xF) == 0) {
                        if ((sp6C == 2) && (sp70 == 0x7F)) {
    ...
    sp30 = temp_a1_2;
    CollisionCheck_SetOC(globalCtx, temp_a1_2, &sp2C->base);
    ...
    if ((s32) this->snuffTimer != 0) {
        ...
        func_800B9010(&this->actor, 0x2031U);
    }
