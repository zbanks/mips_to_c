...
void func_80B04D34(Actor *arg0, GlobalContext *arg1); /* static */
...
typedef struct ObjSnowball {
    ...
    /* 0x01A4 */ void (*actionFunc)(ObjSnowball *, GlobalContext *);
    /* 0x01A8 */ char pad1A8[0x64];                 /* maybe part of actionFunc[26]? */
    /* 0x020C */ f32 unk20C;                        /* inferred */
    /* 0x0210 */ char pad210[0x1];
    /* 0x0211 */ s8 unk211;                         /* inferred */
    /* 0x0212 */ char pad212[0x2];                  /* maybe part of unk211[3]? */
...
void ObjSnowball_Init(Actor *thisx, GlobalContext *globalCtx) {
    ...
    Actor_ProcessInitChain(&this->actor, &D_80B04FD4);
    temp_v1 = sp40 == 1;
    if (temp_v1 != 0) {
        phi_f20 = 1.5f;
    } else {
    ...
    Collider_SetJntSph(globalCtx, temp_a1, &this->actor, &D_80B04F74, &this->unk164);
    ...
    temp_f0 = func_800C411C(&globalCtx->colCtx, &this->actor.floorPoly, &sp44, &this->actor, (Vec3f *) temp_t5);
    ...
    do {
        ...
        if ((phi_s1->unk1D5 & 1) == 0) {
            ...
            if (temp_s0->unk10 <= 0.0f) {
                ...
                SysMatrix_MultiplyVector3fByState(&D_80B04FE4, &sp88);
                temp_f12 = arg0->unk20C * 60.0f * 0.9f;
                phi_f12 = temp_f12;
                if (sp88.y > 0.0f) {
                    sp84 = temp_f12;
                    if (Math3D_AngleBetweenVectors(&D_80B04FD8, &sp88, &sp7C) != 0) {
                        phi_f2 = 1.0f;
                    } else {
