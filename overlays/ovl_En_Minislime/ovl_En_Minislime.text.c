...
void EnMinislime_Init(Actor *thisx, GlobalContext *globalCtx) {
    ...
    this->actor.flags &= -2;
    Collider_InitAndSetCylinder(globalCtx, &this->collider, &this->actor, &D_809857E0);
...
void EnMinislime_AddIceShardEffect(Actor *arg0) {
    ...
    if (temp_v1 < temp_v0) {
        ...
        do {
            ...
            phi_s0->unkC = (f32) (Math_SinS(phi_s3) * temp_f20);
            phi_s0->unk10 = Math_SinS(temp_s1);
            temp_f20_2 = Math_CosS(temp_s1);
            temp_f2 = Math_CosS(phi_s3) * temp_f20_2;
            phi_s0->unk14 = temp_f2;
            phi_s0->unk0 = (f32) (arg0->world.pos.x + (400.0f * arg0->scale.x * phi_s0->unkC));
            phi_s0->unk4 = (f32) (arg0->world.pos.y + (((2.0f * phi_s0->unk10) - 1.0f) * 400.0f * arg0->scale.y));
            phi_s0->unk8 = (f32) (arg0->world.pos.z + (400.0f * arg0->scale.z * temp_f2));
            ...
            phi_s0->unk20 = (f32) ((Rand_ZeroFloat(6.0f) + 2.0f) * 0.001f);
...
void EnMinislime_SetupDisappear(EnMinislime *this) {
    EnMinislime *self = (EnMinislime *) this;
    self->actor.params = 0;
    self->actionFunc = EnMinislime_Disappear;
}
...
void EnMinislime_Disappear(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    temp_v0 = self->actor.params;
    if (temp_v0 == 2) {
        EnMinislime_SetupBreakFromBigslime(self);
        return;
    }
    if (temp_v0 == 1) {
        EnMinislime_SetupFall(self, globalCtx);
        return;
    }
    if (temp_v0 == 9) {
        EnMinislime_SetupDespawn(self);
    }
}
...
void EnMinislime_SetupFall(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    self->collider.base.ocFlags1 |= 1;
    self->actor.speedXZ = 0.0f;
    self->actor.gravity = -2.0f;
    if (EnMinislime_GekkoThrow != self->actionFunc) {
        self->actor.scale.x = 0.095f;
        self->actor.scale.z = 0.095f;
        self->actor.scale.y = 0.10700001f;
        if (Actor_XZDistanceBetweenActors(&self->actor, temp_s1) < 225.0f) {
            temp_v0 = Actor_YawBetweenActors(temp_s1, &self->actor);
            temp_a0 = temp_v0 << 0x10;
            sp22 = temp_v0;
            self->actor.world.pos.x = (Math_SinS((s16) (temp_a0 >> 0x10)) * 225.0f) + temp_s1->world.pos.x;
            self->actor.world.pos.z = (Math_CosS(sp22) * 225.0f) + temp_s1->world.pos.z;
        }
    }
    self->actionFunc = EnMinislime_Fall;
}
...
void EnMinislime_Fall(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    temp_a0 = &self->actor.scale;
    self = self;
    Math_StepToF(&temp_a0->x, 0.17999999f, 0.003f);
    Math_StepToF(&self->actor.scale.y, 0.05f, 0.003f);
    self->actor.scale.z = self->actor.scale.x;
    if ((self->actor.bgCheckFlags & 1) != 0) {
        EnMinislime_SetupGrowAndShrink(self);
    }
}
...
void EnMinislime_SetupBreakFromBigslime(EnMinislime *this) {
    ...
    self->actor.world.rot.y = Actor_YawBetweenActors(self->actor.parent, &self->actor);
    self->actor.shape.rot.y = self->actor.world.rot.y;
    self->actor.speedXZ = Math_CosS(self->actor.world.rot.x) * 15.0f;
    temp_f10 = Math_SinS(self->actor.world.rot.x) * 15.0f;
    self->actor.bgCheckFlags &= 0xFFFE;
    self->actor.gravity = -1.0f;
    self->actor.velocity.y = temp_f10 + 2.0f;
    self->frozenScale = 0.1f;
    self->actor.world.rot.x = Rand_S16Offset(0x800, 0x800);
    self->actor.shape.rot.x = (s16) (Rand_Next() >> 0x10);
    self->actor.shape.rot.z = (s16) (Rand_Next() >> 0x10);
    self->actor.scale.x = 0.15f;
    self->frozenAlpha = 0xC8;
    self->actionFunc = EnMinislime_BreakFromBigslime;
    self->actor.scale.z = 0.15f;
    self->actor.scale.y = 0.075f;
}
...
void EnMinislime_SetupIceArrowDamage(void *arg0) {
    arg0->unk170 = (u8) (arg0->unk170 & 0xFFFE);
    arg0->unk14A = 0x50;
    arg0->unk70 = 0.0f;
    arg0->unk144 = EnMinislime_IceArrowDamage;
    arg0->unk150 = 0.1f;
}
...
void EnMinislime_IceArrowDamage(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    temp_v0 = self->frozenTimer;
    if (temp_v0 == 0x50) {
        self->frozenAlpha += 0xA;
        func_800B9010(&self->actor, 0x20B2U);
        if ((s32) self->frozenAlpha >= 0xC8) {
            self->frozenAlpha = 0xC8;
            self->frozenTimer += -1;
        }
        goto block_16;
    ...
    if ((s32) temp_v0 > 0) {
        self->frozenTimer = temp_v0 - 1;
        temp_v0_2 = self->frozenTimer;
        if (temp_v0_2 == 0x28) {
            Math_Vec3f_Copy(&self->shakeRefPos, &self->actor.world.pos);
            ...
            self->actor.world.pos.x = ((f32) phi_v0 * (sp2C + sp24)) + self->shakeRefPos.x;
            sp24 = Rand_ZeroFloat(sp2C);
            if (Rand_ZeroOne() < 0.5f) {
                phi_v0_2 = -1;
            }
            self->actor.world.pos.z = ((f32) phi_v0_2 * (sp2C + sp24)) + self->shakeRefPos.z;
        }
block_16:
        if ((self->actor.bgCheckFlags & 2) != 0) {
            EnMinislime_AddIceShardEffect(&self->actor);
            EnMinislime_SetupGrowAndShrink(self);
        }
        return;
    }
    EnMinislime_AddIceShardEffect(&self->actor);
    EnMinislime_SetupIdle(self);
}
...
void EnMinislime_SetupFireArrowDamage(void *arg0) {
    ...
    arg0->unkC0 = 0;
    arg0->unk30 = 0;
    arg0->unk171 = (u8) (arg0->unk171 & 0xFFFE);
    arg0->unk144 = EnMinislime_FireArrowDamage;
    arg0->unk70 = 0.0f;
}
...
void EnMinislime_FireArrowDamage(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    if (((s32) temp_v0 % 0x19) == 0) {
        ...
        phi_v0 = self->frozenTimer;
    }
    temp_v1 = phi_v0 * 0xA;
    self->frozenScale = (f32) phi_v0 * 0.0025f;
    if (temp_v1 >= 0xC9) {
        self->frozenAlpha = 0xC8;
    } else {
        self->frozenAlpha = (u8) temp_v1;
    }
    self = self;
    func_800B9010(&self->actor, 0x2144U);
...
void EnMinislime_SetupGrowAndShrink(EnMinislime *this) {
    ...
    self = temp_a2;
    Math_Vec3f_Copy(&temp_a2->actor.home.pos, &temp_a2->actor.world.pos);
    self->frozenTimer = 0x2A;
    self->actor.scale.x = 0.19f;
    self->actor.scale.z = 0.19f;
    self->actor.scale.y = 0.044999998f;
    Audio_PlayActorSound2(&self->actor, 0x38DAU);
    self->actionFunc = EnMinislime_GrowAndShrink;
}
...
void EnMinislime_GrowAndShrink(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    self->actor.scale.x = temp_f2;
    self->actor.scale.z = temp_f2;
    self->actor.scale.y = ((sin_rad((f32) self->frozenTimer * 1.0471976f) * (sp1C * 0.05f)) + 0.75f) * 0.1f;
    if (self->actor.params == 0xA) {
        EnMinislime_SetupMoveToGekko(self);
        return;
    }
    if ((self->actor.xzDistToPlayer < 150.0f) && ((s32) self->frozenTimer < 0x26)) {
        EnMinislime_SetupBounce(self);
        return;
    }
    if (self->frozenTimer == 0) {
        EnMinislime_SetupIdle(self);
...
void EnMinislime_SetupIdle(EnMinislime *this) {
    EnMinislime *self = (EnMinislime *) this;
    self->frozenTimer = 0x14;
    self->collider.base.atFlags |= 1;
    self->collider.base.acFlags |= 1;
    self->actor.params = 3;
    self->actionFunc = EnMinislime_Idle;
}
...
void EnMinislime_Idle(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    self->frozenTimer += -1;
    temp_f0 = sin_rad((f32) self->frozenTimer * 0.31415927f);
    temp_f12 = temp_f0;
    temp_f2 = temp_f0 * 1.5f;
    temp_cond = temp_f2 < 0.0f;
    self->actor.speedXZ = temp_f2;
    if (temp_cond) {
        self->actor.speedXZ = 0.0f;
    } else {
        self->actor.speedXZ = self->actor.speedXZ;
    }
    Math_StepToF(&self->actor.scale.x, ((0.14f * temp_f12) + 1.5f) * 0.1f, 0.010000001f);
    Math_StepToF(&self->actor.scale.y, ((cos_rad((f32) self->frozenTimer * 0.31415927f) * 0.07f) + 0.75f) * 0.1f, 0.010000001f);
    Math_StepToF(&self->actor.scale.z, 0.3f - self->actor.scale.x, 0.010000001f);
    if (self->frozenTimer == 0) {
        temp_a1 = &self->actor.home;
        if (self->actor.xzDistToPlayer < 300.0f) {
            self->actor.world.rot.y = self->actor.yawTowardsPlayer;
        } else {
            sp20 = temp_a1;
            if (Actor_XZDistanceToPoint(&self->actor, &temp_a1->pos) < 200.0f) {
                self->actor.world.rot.y = Actor_YawToPoint(&self->actor, &temp_a1->pos);
            } else {
                self->actor.world.rot.y += Rand_Next() >> 0x13;
            }
        }
        self->frozenTimer = 0x14;
    }
...
void EnMinislime_SetupBounce(EnMinislime *this) {
    EnMinislime *self = (EnMinislime *) this;
    self->actor.speedXZ = 0.0f;
    if (EnMinislime_GrowAndShrink == self->actionFunc) {
        self->frozenTimer = 1;
    } else {
        self->frozenTimer = 4;
    }
    Audio_PlayActorSound2(&self->actor, 0x38D9U);
    self->actionFunc = EnMinislime_Bounce;
}
...
void EnMinislime_Bounce(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    if ((s32) temp_v0 > 0) {
        ...
        Math_StepToF(&self->actor.scale.x, 0.17999999f, 0.010000001f);
        Math_StepToF(&self->actor.scale.y, 0.05f, 0.010000001f);
        goto block_10;
    }
    if (self->actor.velocity.y > 0.0f) {
        Math_StepToF(&self->actor.scale.x, 0.095f, 0.020000001f);
        Math_StepToF(&self->actor.scale.y, 0.10700001f, 0.020000001f);
        goto block_10;
    }
    Math_StepToF(&self->actor.scale.x, 0.17999999f, 0.003f);
    Math_StepToF(&self->actor.scale.y, 0.05f, 0.003f);
    if ((self->actor.bgCheckFlags & 1) != 0) {
        EnMinislime_SetupGrowAndShrink(self);
        return;
...
void EnMinislime_SetupMoveToBigslime(Actor *arg0) {
    ...
    arg0->shape.rot.y = Actor_YawToPoint(arg0, &arg0->parent->home.pos);
    arg0->world.rot.x = (s16) -(s32) arg0->shape.rot.x;
    arg0->unk170 = (u8) (arg0->unk170 & 0xFFFE);
    arg0->unk171 = (u8) (arg0->unk171 & 0xFFFE);
    arg0->unk172 = (u8) (arg0->unk172 & 0xFFFE);
    arg0->scale.x = 0.095f;
    arg0->scale.y = 0.095f;
    arg0->world.rot.y = arg0->shape.rot.y;
    arg0->scale.z = 0.15f;
    if ((s32) arg0->unk148 >= 0x15) {
    ...
    arg0->unk144 = EnMinislime_MoveToBigslime;
}

void EnMinislime_MoveToBigslime(EnMinislime *this, GlobalContext *globalCtx) {
    EnMinislime *self = (EnMinislime *) this;
    if (self->actor.params == 0) {
        EnMinislime_SetupDisappear(self);
        return;
    }
    if ((self->actor.scale.x > 0.0f) && (self->actor.world.pos.y > -190.0f)) {
        self->actor.params = 5;
        self->actor.speedXZ = 0.0f;
...
void EnMinislime_SetupKnockback(Actor *arg0) {
    arg0->unk171 = (u8) (arg0->unk171 & 0xFFFE);
    arg0->unk172 = (u8) (arg0->unk172 | 1);
    arg0->unk14A = 0x1E;
    arg0->speedXZ = 20.0f;
    func_800BE504(arg0, arg0 + 0x160);
    arg0->unk144 = EnMinislime_Knockback;
}
...
void EnMinislime_Knockback(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    temp_f2 = (f32) self->frozenTimer;
    sp2C = sqrtf(temp_f2);
    temp_f14 = 0.05f * sp2C;
    temp_f2_2 = ((cos_rad(temp_f2 * 1.0471976f) * temp_f14) + 1.0f) * 0.15f;
    self->actor.scale.x = temp_f2_2;
    self->actor.scale.z = temp_f2_2;
    if (self->frozenTimer == 0xF) {
        self->collider.base.acFlags |= 1;
    }
    sp24 = temp_f14;
    self->actor.scale.y = ((sin_rad((f32) self->frozenTimer * 1.0471976f) * temp_f14) + 1.0f) * 0.075f;
    if (self->actor.params == 0xA) {
        EnMinislime_SetupMoveToGekko(self);
        return;
    }
...
void EnMinislime_DefeatIdle(EnMinislime *this, GlobalContext *globalCtx) {
    EnMinislime *self = (EnMinislime *) this;
    self->frozenTimer += -1;
    Math_StepToF(&self->actor.scale.x, ((0.14f * sin_rad((f32) self->frozenTimer * 0.31415927f)) + 1.5f) * 0.1f, 0.010000001f);
    Math_StepToF(&self->actor.scale.y, ((cos_rad((f32) self->frozenTimer * 0.31415927f) * 0.07f) + 0.75f) * 0.1f, 0.010000001f);
    Math_StepToF(&self->actor.scale.z, 0.3f - self->actor.scale.x, 0.010000001f);
    if (self->frozenTimer == 0) {
        self->frozenTimer = 0x14;
    }
    if (self->actor.params == 8) {
        EnMinislime_SetupDefeatMelt(self, globalCtx);
    }
}
...
void EnMinislime_SetupDefeatMelt(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    self->actor.velocity.y = -50.0f;
    self->frozenTimer = (s16) (s32) Rand_ZeroFloat(25.0f);
    EnMinislime_AddIceSmokeEffect(self, globalCtx);
    self->actor.params = 0;
    self->actionFunc = EnMinislime_DefeatMelt;
}
...
void EnMinislime_DefeatMelt(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    self->frozenTimer += 1;
    if ((((s32) self->frozenTimer % 0x19) == 0) && ((s32) self->actor.shape.shadowAlpha >= 0x1A)) {
        EnMinislime_AddIceSmokeEffect(self);
    }
    func_800B9010(&self->actor, 0x2144U);
    if (Math_StepToF(&self->actor.scale.y, 0.001f, 0.00075f) != 0) {
        temp_v0 = self->actor.shape.shadowAlpha - 4;
        if (temp_v0 <= 0) {
            self->actor.shape.shadowAlpha = 0;
            EnMinislime_SetupDisappear(self);
        } else {
            self->actor.shape.shadowAlpha = (u8) temp_v0;
        }
    }
    Math_StepToF(&self->actor.scale.x, 0.25f, 0.001f);
    self->actor.scale.z = self->actor.scale.x;
}

void EnMinislime_SetupDespawn(EnMinislime *this) {
    EnMinislime *self = (EnMinislime *) this;
    self->actionFunc = EnMinislime_Despawn;
}
...
void EnMinislime_SetupMoveToGekko(EnMinislime *this) {
    ...
    temp_v0 = self->actor.flags;
    self->actor.speedXZ = 0.0f;
    self->actor.gravity = 0.0f;
    self->actor.velocity.y = 0.0f;
    self->collider.base.acFlags &= -2;
    self->collider.base.ocFlags1 &= -2;
    if ((temp_v0 & 0x2000) == 0x2000) {
        self->actor.flags = temp_v0 & ~0x2000;
    }
    self->actionFunc = EnMinislime_MoveToGekko;
}
...
void EnMinislime_MoveToGekko(EnMinislime *this, GlobalContext *globalCtx) {
    ...
    Math_StepToF(&self->actor.world.pos.x, self->actor.parent->world.pos.x, 10.0f);
    Math_StepToF(&self->actor.world.pos.z, self->actor.parent->world.pos.z, 10.0f);
    Math_StepToF(&self->actor.world.pos.y, self->actor.parent->world.pos.y + 80.0f, 10.0f);
    Math_StepToF(&self->actor.scale.x, 0.15f, 0.0075000003f);
    Math_StepToF(&self->actor.scale.y, 0.075f, 0.0037500001f);
    Math_StepToF(&self->actor.scale.z, 0.15f, 0.0075000003f);
    temp_v0 = self->actor.params;
    if (temp_v0 == 0xB) {
        EnMinislime_SetupGekkoThrow(self);
        return;
    }
    if (temp_v0 == 3) {
        EnMinislime_SetupFall(self, globalCtx);
...
void EnMinislime_SetupGekkoThrow(EnMinislime *this) {
    ...
    self->actor.speedXZ = 17.5f;
    self->actor.world.rot.y = self->actor.yawTowardsPlayer;
    self->actor.gravity = -1.0f;
    self->actor.bgCheckFlags &= 0xFFFE;
    self->frozenTimer = 0xA;
    self->actionFunc = EnMinislime_GekkoThrow;
    self->actor.velocity.y = ((phi_f2 - 200.0f) * 0.01f) + 3.0f;
}

void EnMinislime_GekkoThrow(EnMinislime *this, GlobalContext *globalCtx) {
    EnMinislime *self = (EnMinislime *) this;
    self->frozenTimer += -1;
    self->actor.scale.x = ((0.3f * sin_rad((f32) self->frozenTimer * 0.62831855f)) + 1.5f) * 0.1f;
    self->actor.scale.y = ((cos_rad((f32) self->frozenTimer * 0.62831855f) * 0.2f) + 0.75f) * 0.1f;
    self->actor.scale.z = 0.3f - self->actor.scale.x;
    if (self->frozenTimer == 0) {
        self->frozenTimer = 0xA;
    }
    if ((self->actor.bgCheckFlags & 1) != 0) {
...
void EnMinislime_Update(Actor *thisx, GlobalContext *globalCtx) {
    ...
    if ((temp_v0 == 4) && (EnMinislime_MoveToBigslime != this->actionFunc)) {
        EnMinislime_SetupMoveToBigslime(&this->actor);
        goto block_11;
    }
