...
s32 EffectSsEnIce_Init(GlobalContext *globalCtx, u32 index, EffectSs *this, void *initParamsx) {
    ...
    if (temp_v0 == 0) {
        sp20 = temp_a1;
        Math_Vec3f_Copy(&this->pos, temp_a1);
        temp_v0_2 = initParamsx->unk0;
        if (temp_v0_2 != 0) {
            Math_Vec3f_Diff(&this->pos, temp_v0_2 + 0x24, &this->vec);
        } else {
    ...
    if (temp_v0 == 1) {
        ...
        sp20 = temp_a1_2;
        Math_Vec3f_Copy(&this->pos, temp_a1_2);
...
void func_8097F62C(GlobalContext *globalCtx, u32 index, EffectSs *this) {
    ...
    if ((temp_v0 != 0) && (temp_v0->update != 0)) {
        ...
        if (((s32) temp_v1 >= 9) && (temp_v0->colorFilterTimer != 0) && ((temp_v0->colorFilterParams & 0xC000) == 0)) {
            Math_Vec3f_Sum(&temp_v0->world.pos, &this->vec, &this->pos);
            this->life += 1;
            return;
        }
        if (temp_v1 == 9) {
            temp_f20 = Math_SinS(Math_Vec3f_Yaw(&temp_v0->world.pos, &this->pos));
            this->accel.x = (Rand_ZeroOne() + 1.0f) * temp_f20;
            temp_f20_2 = Math_CosS(Math_Vec3f_Yaw(&this->actor->world.pos, &this->pos));
