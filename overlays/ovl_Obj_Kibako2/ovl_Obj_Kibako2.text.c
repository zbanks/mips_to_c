...
static InitChainEntry D_8098EE8C;                   /* unable to generate initializer */
...
void ObjKibako2_Init(Actor *thisx, GlobalContext *globalCtx) {
    ...
    Collider_SetCylinder(globalCtx, temp_a1, &this->dyna.actor, &D_8098EE60);
    ...
    if ((sp24 != 1) || (ObjKibako2_ContainsSkulltula(&this->dyna.actor, globalCtx) == 0)) {
...
void ObjKibako2_Idle(ObjKibako2 *this, GlobalContext *globalCtx) {
    ObjKibako2 *self = (ObjKibako2 *) this;
    if (ObjKibako2_ShouldBreak(self) != 0) {
        ObjKibako2_Break(self, globalCtx);
        Audio_PlaySoundAtPosition(globalCtx, &self->dyna.actor.world.pos, 0x14, 0x28AAU);
    ...
    if (self->dyna.actor.xzDistToPlayer < 600.0f) {
...
void ObjKibako2_Update(Actor *thisx, GlobalContext *globalCtx) {
    ...
    if ((s32) temp_v0 >= 0) {
        if (temp_v0 == 0) {
            temp_a0 = this;
            this = this;
            Audio_PlayActorSound2(&temp_a0->dyna.actor, 0x39DAU);
