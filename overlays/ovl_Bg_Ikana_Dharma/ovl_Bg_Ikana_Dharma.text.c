...
static InitChainEntry D_80BECAFC;                   /* unable to generate initializer */
...
typedef struct BgIkanaDharma {
    ...
    /* 0x01A8 */ void (*actionFunc)(BgIkanaDharma *, GlobalContext *);
...
void BgIkanaDharma_Init(Actor *thisx, GlobalContext *globalCtx) {
    ...
    Actor_ProcessInitChain(&this->actor, &D_80BECAFC);
    ...
    Collider_SetCylinder(globalCtx, temp_s1, &this->actor, &D_80BECAD0);
...
void func_80BEC5E0(Actor *arg0, GlobalContext *arg1) {
    ...
    if (((arg0->flags & 0x40) == 0x40) && (D_80BECBD0 == 0) && (arg0->xzDistToPlayer < 420.0f)) {
        ...
        if (phi_v0 >= 0x4001) {
            ...
            CollisionCheck_SetAC(arg1, &arg1->colChkCtx, &sp18->base);
        }
    }
}
...
void BgIkanaDharma_Update(Actor *thisx, GlobalContext *globalCtx) {
    ...
    if (func_80BEC5E0 == this->actionFunc) {
        func_800B4AEC(globalCtx, &this->actor, 30.0f);
        ...
        sp28 = phi_f0;
        Actor_SetVelocityAndMoveYRotationAndGravity(&this->actor);
        Actor_UpdateBgCheckInfo(globalCtx, &this->actor, 20.0f, phi_f0, 0.0f, 5U);
    }
