...
s32 func_800E8F08(Vec3s *param_1, Vec3s *param_2) {
    ...
    Math_SmoothStepToS(&param_1->x, 0, 6, 0x1838, (s16) 0x64);
    Math_SmoothStepToS(&param_2->y, 0, 6, 0x1838, (s16) 0x64);
    Math_SmoothStepToS(&param_2->x, 0, 6, 0x1838, (s16) 0x64);
    return 1;
}
...
s32 func_800E8FA4(Actor *actor, Vec3f *param_2, Vec3s *param_3, Vec3s *param_4) {
    ...
    temp_a0 = &actor->focus;
    sp28 = temp_a0;
    sp36 = Math_Vec3f_Pitch(&temp_a0->pos, param_2);
    sp34 = Math_Vec3f_Yaw(&temp_a0->pos, param_2) - actor->world.rot.y;
    Math_SmoothStepToS(&param_3->x, sp36, 6, 0x7D0, (s16) 1);
    temp_a0_2 = param_3->x;
...
block_6:
    if ((temp_t0 != 0) || (D_801D0D50 != 0)) {
        sp2C.x = globalCtx->view.eye.x;
        sp2C.y = globalCtx->view.eye.y;
        sp2C.z = globalCtx->view.eye.z;
    } else {
        sp2C.x = temp_t1->focus.pos.x;
        sp2C.y = temp_t1->focus.pos.y;
        sp2C.z = temp_t1->focus.pos.z;
    }
    func_800E8FA4(actor, &sp2C, param_3, param_4);
    return 1;
}
...
s32 func_800E9250(GlobalContext *globalCtx, Actor *actor, Vec3s *param_3, Vec3s *param_4, ? param_5, ? param_5_unk4, ? param_5_unk8) {
    ...
    temp_t1 = globalCtx->actorCtx.actorList[2].first;
    actor->focus.pos.x = param_5.unk0;
    actor->focus.pos.y = param_5.unk4;
    actor->focus.pos.z = param_5.unk8;
    temp_t0 = globalCtx->csCtx.state;
    if ((temp_t0 == 0) && (D_801D0D50 == 0)) {
        temp_v0 = actor->yawTowardsPlayer - actor->shape.rot.y;
        phi_v1 = temp_v0 << 0x10;
...
block_6:
    if ((temp_t0 != 0) || (D_801D0D50 != 0)) {
        sp24.x = globalCtx->view.eye.x;
        sp24.y = globalCtx->view.eye.y;
        sp24.z = globalCtx->view.eye.z;
    } else {
        sp24.x = temp_t1->focus.pos.x;
        sp24.y = temp_t1->focus.pos.y;
        sp24.z = temp_t1->focus.pos.z;
    }
    func_800E8FA4(actor, &sp24, param_3, param_4);
    return 1;
}
