...
void EffectShieldParticle_Draw(EffShieldParticleParams *params, GraphicsContext *gfxCtx) {
    ...
    if (params != 0) {
        ...
        EffectShieldParticle_CalculateColors(params, &sp194, &sp190);
        temp_a0_15 = gfxCtx->polyXlu.p;
        gfxCtx->polyXlu.p = temp_a0_15 + 8;
        temp_a0_15->words.w0 = 0xFA000000;
        temp_a0_15->words.w1 = (sp194.r << 0x18) | (sp194.g << 0x10) | (sp194.b << 8) | sp194.a;
        temp_a0_16 = gfxCtx->polyXlu.p;
        gfxCtx->polyXlu.p = temp_a0_16 + 8;
        temp_a0_16->words.w0 = 0xFB000000;
        temp_a0_16->words.w1 = (sp190.r << 0x18) | (sp190.g << 0x10) | (sp190.b << 8) | sp190.a;
...
loop_3:
            ...
            SkinMatrix_SetTranslate(&spC0, (f32) params->position.x, (f32) params->position.y, (f32) params->position.z);
            SkinMatrix_SetRotateRPY(&sp100, 0, phi_s1->particles[0].rotationY, 0);
            SkinMatrix_MtxFMtxFMult(&spC0, &sp100, &sp80);
            SkinMatrix_SetRotateRPY(&sp100, 0, 0, phi_s1->particles[0].rotationZ);
            SkinMatrix_MtxFMtxFMult(&sp80, &sp100, &spC0);
            SkinMatrix_SetTranslate(&sp100, temp_f22, 0.0f, 0.0f);
            SkinMatrix_MtxFMtxFMult(&spC0, &sp100, &sp80);
            SkinMatrix_SetScale(&sp100, phi_f20 * 0.02f, 0.02f, 0.02f);
            SkinMatrix_MtxFMtxFMult(&sp80, &sp100, &spC0);
            temp_v0 = SkinMatrix_MtxFToNewMtx(gfxCtx, &spC0);
            if (temp_v0 != 0) {
