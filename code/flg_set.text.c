...
void FlagSet_Draw(GameState *gameState) {
    ...
    GfxPrint_Init(&sp54);
    GfxPrint_Open(&sp54, temp_s0);
    GfxPrint_SetColor(&sp54, 0xFAU, 0x32U, 0x32U, 0xFFU);
    GfxPrint_SetPos(&sp54, 8, 0xD);
    GfxPrint_Printf(&sp54, (&sFlagEntries + (sEntryIndex * 8))->unk4);
    GfxPrint_SetPos(&sp54, 0xC, 0xF);
    D_801ED890 = 7;
    phi_v1 = 7;
    if (7 >= 0) {
        do {
            if (sCurrentBit == phi_v1) {
                GfxPrint_SetColor(&sp54, 0xC8U, 0xC8U, 0xC8U, 0xFFU);
            } else {
                GfxPrint_SetColor(&sp54, 0x64U, 0x64U, 0x64U, 0xFFU);
            }
            if ((**(&sFlagEntries + (sEntryIndex * 8)) & (1 << D_801ED890)) != 0) {
                GfxPrint_Printf(&sp54, "1");
            } else {
                GfxPrint_Printf(&sp54, "0");
            }
            if ((D_801ED890 & 3) == 0) {
                GfxPrint_Printf(&sp54, " ");
            }
    ...
    temp_s0_2 = GfxPrint_Close(&sp54);
    GfxPrint_Destroy(&sp54);
