...
void Actor_ProcessInitChain(Actor *actor, InitChainEntry *ichain) {
    ...
    phi_s0 = ichain;
    do {
        *(&sInitChainHandlers + (((u32) (*phi_s0 * 2) >> 0x1C) * 4))(self, phi_s0);
        phi_s0 += 4;
...
f32 Math_Vec3f_StepTo(Vec3f *start, Vec3f *target, f32 speed) {
    ...
    temp_a1 = start;
    start = start;
    Math_Vec3f_Diff(target, temp_a1, &sp24);
    temp_f0 = Math3D_Vec3fMagnitude(&sp24);
    if (speed < temp_f0) {
        temp_f2 = speed / temp_f0;
        start->x += temp_f2 * sp24.x;
        start->y += temp_f2 * sp24.y;
        start->z += temp_f2 * sp24.z;
        phi_f12 = temp_f0 - speed;
    } else {
